...
public class CheckAdmin {
    public static boolean isChannelAdmin(final Update update, final TelegramLongPollingBot bot) {
        ;
        final GetChatMember member = new GetChatMember();
        member.setChatId(UpdateIdProcess.chatId(update));
        member.setUserId(UpdateIdProcess.userId(update));///userId

        try {
            final ChatMember member1 = bot.execute(member);
